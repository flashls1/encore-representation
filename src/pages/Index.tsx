...
const TalentRow = ({
  ...
  return (
    <Link to={`/talent/${talent.id}`}>
      ...
      >
        {/* Number */}
        <span className="font-mono text-xs w-8 flex-shrink-0 opacity-30" style={{ color: 'var(--accent)' }}>
          {String(index + 1).padStart(2, '0')}
        </span>

        {/* Name */}
        <span className="font-orbitron text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold tracking-wider flex-1 transition-colors duration-300" style={{ color: 'var(--text-primary)' }}>
        ...
        <div className="hidden md:flex items-center gap-2 flex-shrink-0">
          {topRoles.map((r, i) => (
            <span
              ...
              style={{ backgroundColor: 'rgba(212, 175, 55, 0.1)', color: 'var(--accent)', border: '1px solid rgba(212, 175, 55, 0.15)' }}
            >
              {r.character_name}
            </span>
          ))}
        ...
        {/* Arrow */}
        <motion.span className="ml-4 text-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0" style={{ color: 'var(--accent)' }}>
          →
        </motion.span>
      </motion.div>
    </Link>
    ...
    <Link to={`/talent/${talent.id}`}>
      ...
      >
        <div className="relative w-full pb-[100%] overflow-hidden">
          {talent.headshot_url ? (
            <img src={talent.headshot_url} alt={talent.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
          ) : (
            <div className="absolute inset-0 flex items-center justify-center" style={{ backgroundColor: 'var(--bg-elevated)' }}>
              <span className="text-4xl font-bold opacity-30" style={{ color: 'var(--accent)' }}>{initials}</span>
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3">
            <h3 className="font-bold text-sm leading-tight" style={{ color: '#ffffff' }}>{talent.name}</h3>
            {topRoles.length > 0 && (
      ...
      <section ref={sectionRef} className="py-16 sm:py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <div ref={headingRef} className="text-center mb-12 md:mb-16">
            <h2 className="font-orbitron text-3xl sm:text-4xl md:text-5xl mb-3 tracking-wider font-bold" style={{ color: 'var(--accent)' }}>
              Talent Roster
            </h2>
          ...
          {isLoading ? (
            ...
            <>
              {/* Desktop: Interactive name list */}
              <div className="hidden lg:block">
                <div className="border-t" style={{ borderColor: 'rgba(212, 175, 55, 0.08)' }}>
                  {talents.map((talent, i) => (
                    <TalentRow key={talent.id} talent={talent} index={i} onHover={() => setHoveredTalent(talent)} onLeave={() => setHoveredTalent(null)} />
                  ))}
                </div>
                <FloatingPortrait talent={hoveredTalent} mouseX={mousePos.x} mouseY={mousePos.y} />
              </div>

              {/* Mobile/Tablet: Responsive grid */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 lg:hidden">
                {talents.map((talent) => (
                  <motion.div key={talent.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-30px' }} transition={{ duration: 0.4, ease: 'easeOut' }}>
            ...
            <div className="text-center py-16">
              <div className="inline-flex items-center justify-center w-20 h-20 rounded-full mb-4" style={{ backgroundColor: 'var(--badge-bg)' }}>
                <span className="text-3xl" style={{ color: 'var(--accent)' }}>★</span>
              </div>
              <h3 className="text-xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Roster Coming Soon</h3>
              <p style={{ color: 'var(--text-muted)' }}>Our talent roster is being curated. Check back soon!</p>
            </div>
          )}
        </div>
      </section>
